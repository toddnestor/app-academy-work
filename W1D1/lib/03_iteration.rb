...
def factors(num)
  ...
  1.upto(num) {|factor| output << factor if num % factor == 0}
...
class Array
  def bubble_sort!(&prc)
    ...
    return self.bubble_sort!(&prc) if changed
...
def subwords(word, dictionary)
  substrings(word).select do |item|
    dictionary.include?(item)
  end.uniq
end
